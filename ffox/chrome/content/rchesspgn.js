	...
	$(obj).find('.usertext-body').each(function(){
		...
		while (start > -1 && end > -1){
			...
			this.innerHTML = this.innerHTML.replace(
				///\[pgn\][\s\S]*\[\/pgn\]/gim, 
				/\[pgn\][\s\S]*?\[\/pgn\]/im, 
				"<div><div><b><span id='"+id+"-whitePlayer'></span><span id='"+id+"-dash'></span><span id='"+id+"-blackPlayer'></span></b></div><div id='"+id+"-container'></div>" +"<div id='"+id+"-moves' class='rchess-moves'></div></div><div id='"+id+"-piecestaken' class='piecetakencontainer'></div><div style='clear:both; padding-bottom:5px'></div>"
			);
